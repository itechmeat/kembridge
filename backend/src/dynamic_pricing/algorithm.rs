...
use crate::{constants::*, oneinch::OneinchService, price_oracle::PriceOracleService};

use super::{
    price_service::{PriceService, UsdValueEstimator},
    types::*,
};
...
impl DynamicPricingAlgorithm {
    ...
    pub async fn calculate_bridge_price(
        ...
        debug!(
            "Retrieved oracle prices: {} = ${}, {} = ${}",
            from_token, from_price.price, to_token, to_price.price
        );
        ...
        let volatility_adjustment = self
            .calculate_enhanced_volatility_adjustment(
                from_token,
                to_token,
                &base_rate,
                bridge_params,
        ...
        let market_adjustment = self
            .calculate_market_adjustment(from_token, to_token, amount)
            .await?;
        let cross_chain_adjustment = self
            .calculate_cross_chain_adjustment(
                &bridge_params.from_chain,
                &bridge_params.to_chain,
                amount,
            )
            .await?;
    ...
    async fn calculate_enhanced_volatility_adjustment(
        ...
        from_token: &str,
        to_token: &str,
        _base_rate: &BigDecimal,
        bridge_params: &BridgeParameters,
        amount: &BigDecimal,
    ) -> Result<BigDecimal, DynamicPricingError> {
        debug!(
            "Calculating enhanced volatility adjustment for {} -> {} bridge type: {:?}",
        ...
        let bridge_type_adjustment = match bridge_params.bridge_type {
            BridgeType::Atomic => BigDecimal::try_from(BRIDGE_ATOMIC_VOLATILITY_ADJUSTMENT)
                .unwrap_or(BigDecimal::from(1)),
            BridgeType::Optimistic => BigDecimal::try_from(BRIDGE_OPTIMISTIC_VOLATILITY_ADJUSTMENT)
                .unwrap_or(BigDecimal::from(1)),
            BridgeType::Canonical => BigDecimal::try_from(BRIDGE_CANONICAL_VOLATILITY_ADJUSTMENT)
                .unwrap_or(BigDecimal::from(1)),
        };

        // Step 2: Calculate token-pair specific volatility
        let token_volatility = self
            .calculate_token_pair_volatility(from_token, to_token)
            .await?;
        ...
        let size_volatility_adjustment = self
            .calculate_size_based_volatility(amount, from_token)
            .await?;

        // Step 5: Combine all volatility factors
        let final_adjustment = &bridge_type_adjustment
            * &token_volatility
            * &market_volatility_multiplier
            * &size_volatility_adjustment;
    ...
    pub async fn calculate_market_adjustment(
        ...
        debug!(
            "Calculating enhanced market adjustment for {} -> {}",
            from_token, to_token
        );

        // Step 1: Calculate liquidity-based adjustment
        let liquidity_adjustment = self
            .calculate_liquidity_based_adjustment(from_token, to_token, amount)
            .await?;

        // Step 2: Calculate spread-based adjustment
        let spread_adjustment = self
            .calculate_spread_adjustment(from_token, to_token)
            .await?;

        // Step 3: Calculate volume-based adjustment
        let volume_adjustment = self
            .calculate_volume_based_adjustment(from_token, to_token)
            .await?;

        // Step 4: Apply market sentiment adjustment
        let sentiment_adjustment = self
            .calculate_market_sentiment_adjustment(from_token, to_token)
            .await?;

        // Step 5: Combine all market factors
        let market_adjustment =
            &liquidity_adjustment * &spread_adjustment * &volume_adjustment * &sentiment_adjustment;
    ...
    pub async fn calculate_cross_chain_adjustment(
        ...
        debug!(
            "Calculating enhanced cross-chain adjustment for {} -> {}",
            from_chain, to_chain
        );
        ...
        let base_adjustment =
            match (from_chain, to_chain) {
                ("ethereum", "near") => BigDecimal::try_from(BRIDGE_ETH_TO_NEAR_ADJUSTMENT)
                    .unwrap_or(BigDecimal::from(1)),
                ("near", "ethereum") => BigDecimal::try_from(BRIDGE_NEAR_TO_ETH_ADJUSTMENT)
                    .unwrap_or(BigDecimal::from(1)),
                _ => BigDecimal::from(1),
            };

        // Step 2: Apply network congestion multipliers
        let congestion_multiplier = self
            .calculate_congestion_adjustment(from_chain, to_chain)
            .await?;

        // Step 3: Apply security premium based on bridge complexity
        let security_premium = self
            .calculate_security_premium_adjustment(from_chain, to_chain)
            .await?;

        // Step 4: Apply execution time premium
        let execution_time_premium = self
            .calculate_execution_time_premium(from_chain, to_chain)
            .await?;

        // Step 5: Apply amount-based cross-chain adjustment
        let amount_adjustment = self
            .calculate_cross_chain_amount_adjustment(amount, from_chain, to_chain)
            .await?;

        // Step 6: Combine all cross-chain factors
        let final_adjustment = &base_adjustment
            * &congestion_multiplier
            * &security_premium
            * &execution_time_premium
            * &amount_adjustment;
    ...
    async fn calculate_token_pair_volatility(
        ...
        let pair_volatility = match (
            from_token.to_uppercase().as_str(),
            to_token.to_uppercase().as_str(),
        ) {
        ...
        debug!(
            "Token pair volatility for {} -> {}: {:.3}",
            from_token, to_token, pair_volatility
        );
        Ok(pair_volatility)
    }

    /// Calculate market volatility multiplier based on time and conditions
    async fn calculate_market_volatility_multiplier(
        &self,
    ) -> Result<BigDecimal, DynamicPricingError> {
        let hour = chrono::Utc::now().hour();

        // Market volatility patterns based on time (UTC)
        let time_multiplier = if (14..=18).contains(&hour) {
            // Peak US trading hours
        ...
        debug!(
            "Market volatility multiplier at hour {}: {:.3}",
            hour, time_multiplier
        );
        Ok(time_multiplier)
    }
    ...
    async fn calculate_size_based_volatility(
        ...
        let size_adjustment = if estimated_usd_value >= 100000.0 {
            // Very large trades
            BigDecimal::try_from(1.30).unwrap()
        } else if estimated_usd_value >= 50000.0 {
            // Large trades
            BigDecimal::try_from(1.20).unwrap()
        } else if estimated_usd_value >= 10000.0 {
            // Medium trades
            BigDecimal::try_from(1.10).unwrap()
        } else if estimated_usd_value >= 1000.0 {
            // Small trades
            BigDecimal::try_from(1.05).unwrap()
        } else {
            // Micro trades
        ...
        debug!(
            "Size-based volatility for ${:.2}: {:.3}",
            estimated_usd_value, size_adjustment
        );
        Ok(size_adjustment)
    }
    ...
    async fn calculate_time_based_adjustment(&self) -> Result<BigDecimal, DynamicPricingError> {
        ...
        let time_adjustment = if (14..=18).contains(&hour) {
            // Peak hours - higher prices
            BigDecimal::try_from(1.08).unwrap()
        } else if (8..=12).contains(&hour) || (19..=22).contains(&hour) {
            // Busy hours
        ...
        debug!(
            "Time-based adjustment at {}:{:02}: base={:.3}, minute_adj={:.4}, final={:.4}",
    ...
    async fn calculate_liquidity_based_adjustment(
        ...
        debug!(
            "Liquidity adjustment: from={:.3}, to={:.3}, size_impact={:.3}, final={:.3}",
    ...
    async fn calculate_spread_adjustment(
        ...
        debug!(
            "Spread adjustment for {} -> {}: {:.3}",
            from_token, to_token, spread_factor
        );
        Ok(spread_factor)
    }
    ...
    async fn calculate_volume_based_adjustment(
        ...
        let time_volume_factor = if (14..=18).contains(&hour) {
            // Peak volume hours
            BigDecimal::try_from(0.98).unwrap()
        } else if (8..=12).contains(&hour) {
            // Good volume hours
        ...
        debug!(
            "Volume adjustment for {} -> {} at hour {}: base={:.3}, time={:.3}, final={:.3}",
    ...
    async fn calculate_market_sentiment_adjustment(
        ...
        let time_sentiment = if (14..=16).contains(&hour) {
            // Optimistic hours
            BigDecimal::try_from(0.98).unwrap()
        ...
        let weekly_sentiment = if day_of_week == 0 {
            // Monday - cautious
        ...
        debug!(
            "Market sentiment for {} -> {}: time={:.3}, weekly={:.3}, final={:.3}",
    ...
    async fn calculate_congestion_adjustment(
        ...
        debug!(
            "Congestion adjustment {} -> {} at hour {}: from={:.3}, to={:.3}, final={:.3}",
    ...
    async fn calculate_security_premium_adjustment(
        ...
        debug!(
            "Security premium for {} -> {}: {:.3}",
            from_chain, to_chain, security_premium
        );
        Ok(security_premium)
    }
    ...
    async fn calculate_execution_time_premium(
        ...
        debug!(
            "Execution time premium for {} -> {}: {:.3}",
            from_chain, to_chain, time_premium
        );
        Ok(time_premium)
    }
    ...
    async fn calculate_cross_chain_amount_adjustment(
        ...
        let amount_adjustment = if estimated_usd >= 100000.0 {
            // Very large amounts
            BigDecimal::try_from(1.12).unwrap()
        } else if estimated_usd >= 50000.0 {
            // Large amounts
            BigDecimal::try_from(1.08).unwrap()
        } else if estimated_usd >= 10000.0 {
            // Medium amounts
            BigDecimal::try_from(1.04).unwrap()
        } else if estimated_usd >= 1000.0 {
            // Small amounts
            BigDecimal::try_from(1.02).unwrap()
        } else {
            // Micro amounts
        ...
        debug!(
            "Cross-chain amount adjustment for ${:.2} ({} -> {}): {:.3}",
