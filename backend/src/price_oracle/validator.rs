...
use crate::price_oracle::types::{PriceData, PriceError, ValidationResult, TradingPair};
...
impl PriceValidator {
    ...
    pub fn validate_price(&self, price: &PriceData) -> Result<PriceData, PriceError> {
        ...
        if !validation_result.is_valid {
            return Err(PriceError::ValidationFailed(
                validation_result.reason.unwrap_or("Unknown validation error".to_string())
            ));
        }
        ...
        if self.validation_rules.enable_anomaly_detection {
            if let Some(anomaly_score) = self.detect_anomaly(price) {
                if anomaly_score > self.validation_rules.anomaly_threshold {
                    warn!("Anomaly detected for {} with score {}", price.symbol, anomaly_score);
                    let mut adjusted_price = price.clone();
    ...
    fn validate_basic_rules(&self, price: &PriceData) -> Result<ValidationResult, PriceError> {
        ...
        // Check staleness
        let age_seconds = Utc::now().signed_duration_since(price.timestamp).num_seconds();
    ...
    fn validate_advanced_rules(&self, price: &PriceData) -> Result<(), PriceError> {
        ...
        if let Some(pair) = TradingPair::from_symbol(&price.symbol) {
            match pair {
                ...
                TradingPair::NearUsd => {
                    if price.price < Decimal::from_f64(0.1).unwrap() || price.price > Decimal::from(100) {
                        return Err(PriceError::ValidationFailed(
                ...
                TradingPair::UsdtUsd | TradingPair::UsdcUsd => {
                    if price.price < Decimal::from_f64(0.95).unwrap() || price.price > Decimal::from_f64(1.05).unwrap() {
                        return Err(PriceError::ValidationFailed(
        ...
        if let Some(change_24h) = price.change_24h {
            if change_24h.abs() > self.validation_rules.max_change_percent {
                return Err(PriceError::ValidationFailed(
                    format!("24h change too extreme: {}%", change_24h)
                ));
            }
        }
        ...
        match price.source.as_str() {
            "chainlink" => {
                if price.confidence < 0.8 {
                    return Err(PriceError::ValidationFailed(
                        "Chainlink confidence too low".to_string()
                    ));
                }
            }
            "coingecko" => {
            ...
            _ => {
                return Err(PriceError::ValidationFailed(
                    format!("Unknown price source: {}", price.source)
                ));
            }
        }
    ...
    fn detect_anomaly(&self, price: &PriceData) -> Option<f64> {
        if let Some(historical) = self.historical_prices.get(&price.symbol) {
            ...
            let recent_prices: Vec<Decimal> = historical.iter()
                .rev()
                .take(10) // Last 10 prices
                .map(|p| p.price.clone())
                .collect();
            
            let mean = recent_prices.iter().sum::<Decimal>() / Decimal::from(recent_prices.len() as i32);
            ...
            let z_score = if std_dev > 0.0 { current_diff / std_dev } else { 0.0 };
    ...
    pub fn add_to_history(&mut self, price: &PriceData) {
        let history = self.historical_prices.entry(price.symbol.clone())
    ...
    pub fn validate_multiple_prices(&self, prices: &[PriceData]) -> Vec<Result<PriceData, PriceError>> {
        prices.iter()
    ...
    pub fn get_validation_stats(&self) -> ValidationStats {
        ...
        for (symbol, history) in &self.historical_prices {
            let symbol_stats = SymbolValidationStats {
                ...
                avg_confidence: history.iter().map(|p| p.confidence).sum::<f64>() / history.len() as f64,
            ...
            CircuitBreakerState::Open => {
                if let Some(last_failure) = self.last_failure_time {
                    if Utc::now().signed_duration_since(last_failure) > self.recovery_timeout {
                        ...
                        Err(PriceError::ValidationFailed("Circuit breaker is open".to_string()))
                    }
                } else {
                    Err(PriceError::ValidationFailed("Circuit breaker is open".to_string()))
                }
