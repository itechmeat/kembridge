...
export const SecurityAlerts: FC<SecurityAlertsProps> = ({
  alerts,
  maxVisible = 5,
  position = "top",
  onDismiss,
  ...
  return (
    <div className={cn(styles.securityAlerts, styles[position])}>
      <div className={styles.container}>
        {visibleAlerts.map((alert) => (
          <div
            key={alert.id}
            className={cn(styles.alert, styles[alert.priority], {
              [styles.dismissing]: dismissingAlerts.has(alert.id),
            })}
            data-testid="security-warning"
          >
            <div className={styles.content}>
              <div className={styles.alertHeader}>
                <span className={styles.alertIcon}>{getAlertIcon(alert)}</span>
              ...
              {alert.actions && alert.actions.length > 0 && (
                <div className={styles.actions}>
                  {alert.actions.map((action) => (
                    <button
                      key={action.id}
                      className={cn(styles.action, styles[action.type])}
