...
} from "../../services/ai/aiRiskService";
...
export function useAIRiskAnalysis(params: UseAIRiskAnalysisParams) {
  ...
  // Subscribe to AI Engine health status
  useEffect(() => {
    const unsubscribe = AIRiskService.subscribeToHealthStatus((isHealthy) => {
      setState((prev) => ({ ...prev, isAIEngineHealthy: isHealthy }));
    });
   ...
   */
  const analyzeTransactionRisk = useCallback(
    async (transaction: {
      ...
      setState((prev) => {
        if (!prev.isAIEngineHealthy) {
          const error = new Error(
            "AI Engine is not available for risk analysis"
          );
          onError?.(error);
          return { ...prev, error };
        }
      ...
      try {
        ...
        const err =
          error instanceof Error ? error : new Error("Risk analysis failed");
        setState((prev) => ({
          ...prev,
          isAnalyzing: false,
          error: err,
   ...
   */
  const checkAddressBlacklist = useCallback(
    async (address: string, chain: string) => {
      if (!state.isAIEngineHealthy) {
        const error = new Error(
          "AI Engine is not available for blacklist check"
        );
        onError?.(error);
        return null;
      }
      ...
      try {
        ...
        const err =
          error instanceof Error ? error : new Error("Blacklist check failed");
        onError?.(err);
        return null;
      }
    },
  ...
  const getUserRiskProfile = useCallback(async () => {
    ...
    try {
      ...
      const err =
        error instanceof Error ? error : new Error("User profile fetch failed");
      onError?.(err);
      return null;
    }
  ...
  const getRiskFactors = useCallback(() => {
    ...
    return {
      ...
      recommendations: state.currentRisk.recommended_action
        ? [state.currentRisk.recommended_action]
        : [],
