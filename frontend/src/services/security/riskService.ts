...
export class RiskService {
   ...
   */
  static async analyzeTransactionRisk(
    ...
    try {
      ...
      return {
        ...
        blacklistStatus: {
          ...
          reason:
            Math.random() > 0.95
              ? "Address flagged by security provider"
              : undefined,
        },
        analysis: {
  ...
  static async getUserRiskProfile(
    userId?: string
  ): Promise<UserRiskProfileResponse["data"]> {
    try {
      const endpoint = userId
        ? `${API_ENDPOINTS.RISK.PROFILE_BY_ID}/${userId}`
        : API_ENDPOINTS.RISK.PROFILE;

      const response = await apiClient.get<UserRiskProfileResponse>(endpoint);
      return response.data;
    } catch (error) {
      console.warn(
        "User risk profile API not available, using mock data",
        error
      );
      ...
      return {
        ...
        high_risk_transactions: Math.floor(Math.random() * 10),
  ...
  static async checkAddressBlacklist(
    ...
    try {
      ...
      const response = (await apiClient.post(
        API_ENDPOINTS.RISK.BLACKLIST_CHECK,
        {
          address,
          chain,
        }
      ...
      return {
        ...
        reason:
          Math.random() > 0.98 ? "Flagged by security provider" : undefined,
        confidence: 0.8,
      };
    }
  }
