...
- [ ] **1.2.1** Создание базовой схемы users таблицы
- [ ] **1.2.2** Создание user_auth_methods таблицы для Web3 авторизации
- [ ] **1.2.3** Создание user_sessions таблицы для JWT токенов
- [ ] **1.2.4** Создание transactions таблицы для кросс-чейн операций
- [ ] **1.2.5** Создание quantum_keys таблицы для постквантовых ключей
- [ ] **1.2.6** Создание audit_logs таблицы для мониторинга
- [ ] **1.2.7** Настройка индексов для производительности
- [ ] **1.2.8** Создание миграций с помощью sqlx или diesel

### 1.3 Basic API Gateway (Rust/Axum)

- [ ] **1.3.1** Инициализация Axum проекта с базовой структурой
- [ ] **1.3.2** Настройка CORS для фронтенда
- [ ] **1.3.3** Подключение к PostgreSQL через sqlx
- [ ] **1.3.4** Подключение к Redis для кеширования
- [ ] **1.3.5** Создание базовых роутов (/health, /api/v1)
- [ ] **1.3.6** Настройка логирования (tracing)
- [ ] **1.3.7** Добавление middleware для обработки ошибок
- [ ] **1.3.8** Создание основных структур данных (User, Session, etc.)

**Checkpoint 1.3:** Axum backend работает в Docker, подключен к PostgreSQL/Redis, отвечает на /health
...
- [ ] **2.1.1** Создание модуля для работы с Web3 подписями
- [ ] **2.1.2** Реализация генерации nonce для подписи
- [ ] **2.1.3** Создание endpoint POST /api/auth/nonce
- [ ] **2.1.4** Реализация верификации Ethereum подписей (secp256k1)
- [ ] **2.1.5** Реализация верификации NEAR подписей (ed25519)
- [ ] **2.1.6** Создание endpoint POST /api/auth/verify-wallet
- [ ] **2.1.7** Интеграция с user_auth_methods таблицей
- [ ] **2.1.8** Тестирование авторизации для обеих сетей

### 2.2 JWT Session Management

- [ ] **2.2.1** Настройка JWT библиотеки (jsonwebtoken)
- [ ] **2.2.2** Создание структуры JWTClaims с необходимыми полями
- [ ] **2.2.3** Реализация генерации JWT токенов
- [ ] **2.2.4** Реализация верификации JWT токенов
- [ ] **2.2.5** Создание middleware для аутентификации
- [ ] **2.2.6** Сохранение активных сессий в user_sessions
- [ ] **2.2.7** Реализация logout функциональности
- [ ] **2.2.8** Добавление refresh token логики

### 2.3 User Management

- [ ] **2.3.1** Создание endpoint GET /api/user/profile
- [ ] **2.3.2** Реализация создания пользователей при первом входе
- [ ] **2.3.3** Связывание wallet addresses с пользователями
- [ ] **2.3.4** Поддержка множественных кошельков для одного пользователя
- [ ] **2.3.5** Создание endpoint PUT /api/user/profile для обновления данных
- [ ] **2.3.6** Реализация мягкого удаления пользователей
- [ ] **2.3.7** Добавление базовой валидации данных пользователя

**Checkpoint 2.3:** Полная Web3 авторизация работает, пользователи могут войти через MetaMask/NEAR Wallet
...
- [ ] **4.2.6** Создание смарт-контракта для моста на NEAR
- [ ] **4.2.7** Реализация кросс-чейн вызовов
- [ ] **4.2.8** Интеграция с NEAR 1Click API для упрощенного UX
- [ ] **4.2.9** Реализация автоматической оптимизации маршрутов через 1Click
- [ ] **4.2.10** Добавление мониторинга NEAR транзакций
- [ ] **4.2.11** Тестирование Chain Signatures функциональности
...
- [ ] **9.3.6** Документирование всех endpoints в OpenAPI
